...
- Interactive `sftp` completion remains client-side and works normally through the proxy because CentralSSH forwards the subsystem stream without injecting a pseudo-shell.
...
- Linux packaging produces `centralssh-<version>-linux-<arch>-systemd.tar.gz`, `centralssh-<version>-linux-<arch>-openrc.tar.gz`, matching `.deb` packages, and matching `.rpm` packages for both `amd64` and `arm64`.
- FreeBSD amd64 packaging now runs on the native `freebsd-15` runner and produces `centralssh-<version>-freebsd-15.0-RELEASE-amd64.pkg` and `centralssh-<version>-freebsd-15.0-RELEASE-amd64.tar.gz`. The filename carries the target FreeBSD release explicitly. When privilege is available, the runner validates the installed package through the packaged rc script with a temporary root-owned config tree.
- FreeBSD aarch64 packaging now also runs on the native `freebsd-15` runner as a cross-build and produces `centralssh-<version>-freebsd-15.0-RELEASE-aarch64.pkg` and `centralssh-<version>-freebsd-15.0-RELEASE-aarch64.tar.gz`. The filename carries the target FreeBSD release explicitly. The cross build uses the FreeBSD `aarch64` sysroot wrapper and `cargo +nightly -Z build-std`.
