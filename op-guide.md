...
- Linux and FreeBSD jobs run on their native runner classes. Package jobs stage only their validated artifacts under `RELEASE_STAGING_ROOT`, keyed by repository, tag, and commit.
- Tagged runs publish from the final `release-publish` job only after every required Linux and FreeBSD package job succeeds. That job collects the expected artifacts into one workspace, writes a single `sha256sums` file, uploads all assets, and publishes one Forgejo release.
