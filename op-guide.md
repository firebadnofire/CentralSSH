...
- Linux and FreeBSD jobs run on their native runner classes. Package jobs upload only their validated artifacts as CI artifacts.
- Tagged runs publish from the final `release-publish` job only after every required Linux and FreeBSD package job succeeds. That job downloads the expected CI artifacts into one workspace, writes a single `sha256sums` file, uploads it beside the artifacts, and publishes one Forgejo release.
